...
ourApp.get("/publication/:pubID",async (req,res) =>{
    ...
    const specificpub = await PublicationModel.findOne({id: parseInt(pubID) });
    ...
    return res.json({pubication : specificpub});
});
...
ourApp.get("/book/a/:author",async (req,res) =>{
    const {author} = req.params;
    
    const BookbyAuthor = await BookModel.findOne({authors: parseInt(author)});
    return res.json({book : BookbyAuthor});
});
...
ourApp.get("/author/:authorID",async (req,res) =>{
    ...
    const getAuthor = await AuthorModel.findOne({id : parseInt(authorID)});
